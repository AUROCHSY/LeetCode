...
|:---:|:---:|:---:|:---:|
| cpp  | 4 ms | 8.9 MB | 2021/03/29 12:02 |

```cpp

/*
当前解法：双指针, 从前往后，将nums1和nums2归并到辅组数组numsTmp，然后再从numsTmp复制回nums1(题目要求通过nums1返回)。时间复杂度为O(m+n)，空间复杂度为O(m+n);
优化解法1：双指针，从前往后，先将输入数组nums1复制到辅助数组numsTmp,然后将nums2和numsTmp归并到nums1。时间复杂度为O(m+n)，空间复杂度为O(m);
最优解法：双指针，从后往前。因为nums1的总长度是已知的,且nums1后面是空的。可以将nums1和nums2从后往前归并到nums1，这样空间复杂度只需要O(1)
*/
class Solution {
public:
